...
def export_data(data_rows: list, output_filepath: str):
    ...
    if not os.path.isfile(output_filepath):
        with open(output_filepath, 'w', newline='') as f:
            writer = csv.writer(f)  
            writer.writerow(['author_id', 'repository', 'commit_id', 'date', 'filepath', 'method', 'added_line'])
...
def get_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--author_id', '-a',
                        ...
                        required=True,
                        type=int,
                        help='ID of the developer from which generate the corpus')
    return parser
...
if __name__ == '__main__':
    ...
    author_df = commits_df[commits_df['author_id'] == args.author_id].sort_values(by=['repository'])

    # Save JSON files by repository
    repos_dict = defaultdict(list)
    repositories, data_id_list = author_df['repository'].tolist(), author_df['data_id'].tolist()
    commits, dates = author_df['commit_id'].tolist(), author_df['date'].tolist()
    for repo, data_id, commit, date in zip(repositories, data_id_list, commits, dates):
        json_path = os.path.join(args.ext_dir, f'ext_{str(int(data_id))}.json')
        repos_dict[repo].append((json_path, commit, date))
    ...
    for repo in repos_dict:
        ...
        Repo.clone_from(f'https://github.com/apache/{repo}.git', clone_dir)
        
        exported_rows = list()
        repo_data = repos_dict[repo]
        for path, commit, date in repo_data:
            ...
            for file in changed_files:
                ...
                for method, indexes in zip(methods, added_indexes):
                    method_data = [args.author_id, repo, commit, date, file.filename, method, indexes]
                    exported_rows.append(method_data)
        
        # Export data
        if not os.path.exists(args.output_dir): os.makedirs(args.output_dir)
        output_filepath = os.path.join(args.output_dir, 'extracted.csv')
