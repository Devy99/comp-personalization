...
for (dev in devs) {
    ...
    for (model in models) {
        model_col=read.csv(paste0(datasets_path, dev, '/', model, '/predictions_test.csv'))$correct
        baseline_col=read.csv(paste0(datasets_path, dev, '/pt-sft1/predictions_test.csv'))$correct
