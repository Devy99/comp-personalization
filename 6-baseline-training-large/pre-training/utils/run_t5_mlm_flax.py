...
class DataTrainingArguments:
    ...
    enable_early_stopping: bool = field(
        default=False,
        metadata={"help": "Enable to stop training according to the early stopping strategy"}
    )

    delta_value: float = field(
        default=0.01,
        metadata={"help": "The delta value to apply during early stopping"},
    )

    masked_validation_batches: str = field(
        default=None,
        metadata={"help": "Directory path of the binary file containing the list of batches with the masked dataset. If specified, the script will not perform the on-the-fly masking. \
                           A preliminary check will be performed to ensure that the evaluation batch size is identical to the one used for generating the batches."}
    )

    metrics_filename: Optional[str] = field(
        default="eval_results.json",
        metadata={"help": "Name of the JSON file where to store the evaluation metrics."},
    )

    save_predictions: Optional[str] = field(
        default=None,
        metadata={"help": "If enabled, it will save the model predictions for each bach to the specified directory path (for evaluation) "},
    )
...
def get_best_checkpoint(checkpoints, accuracies, delta):
  ...
  print(f'Best checkpoint {best_check}, Accuracy {best_acc}')
  return best_check, continue_training
...
def main():
    ...
    def eval_step(params, batch):
        ...
        if data_args.save_predictions:
            num_occ = len(glob.glob('masked_ds_predicted_*'))
            filename = f'masked_ds_predicted_{num_occ}.txt'
            output_dir = os.path.join(data_args.save_predictions, filename)
    ...
    # Inject an already masked dataset in batches
    if data_args.masked_validation_batches:

        # Check if the batch size is similar to the one used for the masking
        info_path = os.path.join(data_args.masked_validation_batches, 'masking_info.json')
        with open(info_path, 'r') as f:
            info_dict = json.load(f)
            assert eval_batch_size == info_dict['eval_batch_size'], f'The specified batch size ({eval_batch_size}) does not match the one used for masking the dataset ({str(info_dict["eval_batch_size"])})'

        # Retrieve the masked batches
        bin_path = os.path.join(data_args.masked_validation_batches, 'masked_batches.bin')
        with open(bin_path,'rb') as f: 
            masked_batches = pickle.load(f)
    ...
    if training_args.do_train:
        ...
        checkpoints, accuracies = list(), list()
        for epoch in epochs:
            # ======================== Training ================================
            ...
            for step, batch_idx in enumerate(tqdm(train_batch_idx, desc="Training...", position=1)):
                ...
                if cur_step % training_args.eval_steps == 0 and cur_step > 0:
                    ...
                    for i, batch_idx in enumerate(tqdm(eval_batch_idx, desc="Evaluating ...", position=2)):
                        ...
                        # Generate the masked dataset for the selected batch
                        model_inputs = None
                        if data_args.masked_validation_batches:
                            model_inputs = masked_batches[i]
                        else:
                ...
                if cur_step % training_args.save_steps == 0 and cur_step > 0:
                    ...
                    if jax.process_index() == 0:
                        ...
                        dir_name = os.path.join(training_args.output_dir, f't5-pretrain-checkpoint-{cur_step}')

                        # Save checkpoint
                        save_checkpoint(model, dir_name, state, cur_step, with_opt=True, push_to_hub=training_args.push_to_hub)

                        # Check if continuing training
                        if data_args.enable_early_stopping:
                            best_checkpoint, continue_training = get_best_checkpoint(checkpoints, accuracies, data_args.delta_value)
                            if not continue_training: exit()
    ...
    if training_args.do_eval:
        ...
        if jax.process_index() == 0:
            ...
            path = os.path.join(training_args.resume_from_checkpoint, data_args.metrics_filename)
